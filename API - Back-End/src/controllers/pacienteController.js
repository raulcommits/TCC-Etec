...
route.post("/", async (request, response) => {
    const {cpf, sus, nome, data_nascimento, num_telefone, email, 
    etnia, genero, escolaridade, nacionalidade, naturalidade_estado, naturalidade_municipio,
    estado_clinico, responsavel_legal, filiacao_mae, filiacao_pai, id_endereco, id_profissional, id_cbo} = request.body;
    ...
    try {
        ...
        const profissional = await repositorioProfissional.findOneBy({
            id: id_profissional,
            deletedAt: IsNull()
        })
        if(!profissional) {
            return response.status(400).send({"response": "Esse profissional não foi encontrado."});
        }
        ...
        const novo_paciente = repositorioPaciente.create({cpf, sus, nome, data_nascimento, num_telefone,
        email, etnia, genero, escolaridade, nacionalidade, naturalidade_estado, naturalidade_municipio,
        estado_clinico, responsavel_legal, filiacao_mae, filiacao_pai, endereco, profissional, cbo});
