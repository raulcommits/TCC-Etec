...
route.get("encontrarBairro", async (request, response) => {
    const {encontrarBairro} = request.params;
    const encontrarZona = await repositorioZona.findBy({nome_zona: Like(`%${encontrarBairro}`)});
